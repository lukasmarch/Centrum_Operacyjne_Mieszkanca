...
_INCIDENTS: tuple[tuple[str, str, str], ...] = (
    (
        "prad",
        "Wyłączenie prądu",
        r"wylaczeni\w*\s+(pradu|planowan|biezac)|przerw\w*\s+w\s+dostaw\w*\s+(energii|pradu)"
...
# `\b` po nazwie odpadałoby przy odmianie („w Rumianie", „do Żabin"), więc
# dopuszczamy końcówkę fleksyjną — ale nie dowolną, żeby „Prusy" nie łapało
# „Prusaka", a „Wery" — „weryfikacji".
_PLACE_RE = tuple(
    (place, re.compile(rf"\b{_flat(place).replace(' ', r'\s+')}(a|ach|ami|ą|e|ę|ie|ii|om|ow|ow|u|y|ych|ym|ie)?\b"))
    for place in GMINA_RYBNO_PLACES
)
