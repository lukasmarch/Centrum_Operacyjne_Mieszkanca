    ...
    return (
        <div className="mt-2 pt-4 border-t border-white/5">
            ...
            <div className="relative flex justify-between items-center px-4 mb-6 h-6">
                {/* Track Background */}
                <div className="absolute left-6 right-6 h-[4px] bg-slate-700/50 z-0 rounded-full" />
                    ...
                    if (isStart || isEnd) {
                        return (
                            <div key={id} className="relative z-10 flex flex-col items-center">
                                ...
                                <span className={`absolute -bottom-5 whitespace-nowrap text-[8px] font-bold uppercase tracking-tight ${isPassed || isCurrent || (isStart && index <= currentIndex) ? 'text-blue-400' : 'text-slate-500'
                                    }`}>
                                    {stop?.name.split(' ')[0]}
                                </span>
                            </div>
                        ...
                        <div key={id} className="relative z-10 flex flex-col items-center">
                            <div
                                className={`w-2 h-2 rounded-full border-[1.5px] transition-all duration-500 ${isPassed ? 'bg-blue-500 border-blue-500' :
                                    isCurrent ? 'bg-slate-900 border-blue-500 ring-2 ring-blue-500/30 scale-125 shadow-md' :
        ...
        <div className={`flex-1 p-4 rounded-2xl border transition-all duration-500 ${status.isActive ? 'bg-blue-500/10 border-blue-500/20 shadow-sm ring-1 ring-blue-500/10' : 'bg-white/5 border-white/5'
            ...
            <div className="flex justify-between items-start mb-3">
                <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                        <div className={`w-1.5 h-1.5 rounded-full ${status.isActive ? 'bg-blue-500 animate-pulse' : 'bg-slate-600'}`} />
                        <p className={`text-[10px] font-bold uppercase tracking-tight ${status.isActive ? 'text-blue-400' : 'text-slate-500'}`}>
            ...
            {status.isActive && bus ? (
                <div className="mt-3">
                    ...
                    <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden shadow-inner relative">
                        <div
                ...
                <div className="mt-3">
                    <p className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Następny:</p>
                    <div className="flex items-baseline gap-1.5 mt-1">
                        <span className="text-xl font-black text-slate-200 tabular-nums tracking-tighter">{next.time}</span>
                        <span className="text-[10px] font-bold text-slate-500 uppercase">za {next.inMinutes}m</span>
                    </div>
                </div>
            ) : (
            ...
            <div className="flex justify-between items-start px-1">
                <div>
                    <div className="flex items-center gap-2.5 mb-1">
                        ...
                        <h3 className="text-lg font-bold text-slate-100 leading-none">Monitoring</h3>
                    </div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider ml-10 pl-0.5">Lokalizacja Live</p>
                </div>
                <div className="flex flex-col items-end pt-0.5">
                    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg shadow-sm border ${activeBuses.length > 0 ? 'bg-blue-600 border-blue-500' : 'bg-slate-800 border-slate-700'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${activeBuses.length > 0 ? 'bg-white animate-pulse' : 'bg-slate-500'}`} />
                        <span className={`text-[9px] font-bold uppercase tracking-tight ${activeBuses.length > 0 ? 'text-white' : 'text-slate-400'}`}>
                            {activeBuses.length > 0 ? 'Aktywny' : 'Czuwanie'}
                        </span>
                    </div>
                    <span className="text-[10px] text-slate-500 mt-1.5 font-bold tabular-nums">{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
            </div>
